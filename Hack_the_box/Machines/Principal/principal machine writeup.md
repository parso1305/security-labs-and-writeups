...
* Port 22: SSH
* Port 8080: Web application (Jetty)
The HTTP response headers included:
...
* If the inner JWT is unsigned (alg: none), signature verification is skipped
This meant I could:
1. Create my own JWT
...
However, the system lacked proper principal validation:
* No restriction on which usernames are allowed
...
2. Not recognizing token type
I tried to decode the token directly, not realizing it was JWE (encrypted).
Lesson: Identify whether a token is readable (JWT) or encrypted (JWE).
________________


3. Misunderstanding key usage
I attempted to manually construct the RSA key instead of using the JWKS response.
Lesson: Use provided cryptographic material as-is.
________________


4. Overcomplicating the exploit
I initially assumed the attack would be complex, when it was actually a straightforward logic flaw.
Lesson: Simpler paths are often correct in CTF-style environments.
________________


Core Insight
Both stages of this machine exploit the same fundamental issue:
The system validates cryptographic correctness but fails to validate identity.
* In the web app:
   * Token is validly encrypted
   * Identity is not verified
* In SSH:
   * Certificate is validly signed
   * Principal is not verified
________________
